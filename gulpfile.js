...
export function optimizePhoto() {
  const PHOTO_TYPE = ['small','mobile','tablet','desktop'];
  const PHOTO_WIDTH = [100, 200, 400, 800];
  const PHOTO_HEIGHT = [150, 301, 601, 1202];
  const RAW_DENSITY = 2;
  const TARGET_FORMATS = [undefined, 'webp']; // undefined — initial format: jpg or png
      ...
      for (const format of TARGET_FORMATS) {
        ...
        for (let density = RAW_DENSITY; density > 0; density--) {
          formats.push(
            {
              format,
              rename: { suffix: `-${PHOTO_TYPE[type]}@${density}x` },
              // width: Math.ceil(PHOTO_SIZE[type] * density / RAW_DENSITY),
              width: (metadata) => {
                if (metadata.width > metadata.height) {
                  return Math.ceil(PHOTO_HEIGHT[type] * density);
                } else {
                  return Math.ceil(PHOTO_WIDTH[type] * density);
                }
              },
              jpegOptions: { progressive: true },
            },
  ...
  return src(`${PATH_TO_RAW}photo/**/*.{png,jpg,jpeg}`)
    .pipe(sharp(createOptionsFormat()))
    .pipe(dest(`${PATH_TO_SOURCE}images/photo`));
}
